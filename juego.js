...
function endGame(){
	...
	if (sonTablas){
		alert("Fin del juego. Juego en tablas"); 
	}
	else if (turnoBlancas){
		alert("Fin del juego. Ganan Negras"); 
	}
	else {
		alert("Fin del juego. Ganan Blancas"); 
	}
	newGame();
}
...
function getLegalMovesPieza(unaPieza){
	...
	while (i <2){
		if (((unaPieza.row != 0)&&(turnoBlancas))||((unaPieza.row != 7)&&(turnoNegras))){ // Si estan al final del tablero, no hay movimientos posibles
			if (((unaPieza.column != 0)&& (i==-1))||((unaPieza.column != 7)&& (i==1))){ // Si estan en una esquina del tablero, solo hay que comprobar uno de los laterales
				if (turnoBlancas){ // Asi controlamos la direccion de la pieza
					fila = unaPieza.row -1;
					...
					if ((piezas[j].row == fila) && (piezas[j].column == columna)){
						...
						if (piezas[j].color != unaPieza.color){ // Si son de distinto color, igual se puede saltar
							if ((i<0)&&(turnoBlancas)&&(unaPieza.column >= 2)&&(unaPieza.row >= 2)){ // Miramos si, siendo blancas, tienen sitio para saltar 
								...
								vacia = casillaVacia(fila, columna); // Si tiene sitio y esta vacia, hay sitio para hacer un salto
							}
				...
				if ((existe == false)){ // Si la casilla contigua esta libre, se puede mover.
					var aMove = new Move(unaPieza.row, unaPieza.column, fila, columna); 
					someLegalMoves.push(aMove); 
				}
				else if ((existe ==true) && (vacia==true)){  //Si no esta libre pero se puede hacer un salto, tambi�n.
...
function clickOnEmptyCell(cell) {
	...
	if ((rowDiff == 1 && Math.abs(columnDiff) == 1) && (!(legalMoves[0] instanceof Jump))){
		...
		// De momento, esto es asi hasta que se puedan hacer saltos en cadena. 
		gSelectedPieceIndex = -1;
		gSelectedPieceHasMoved = false;
